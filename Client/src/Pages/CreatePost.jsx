...
const CreatePost = () => {
  ...
  const handleSubmit =async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://dall-e-07kz.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
      ...
      alert('please enter an promt and generate an image')
    }
  ...
  const generateImage = async (e) => {
    if (form.prompt) {
      try {
        ...
        // const response = await axios.post('http://localhost:3015/api/v1/dalle', { obj })
        const response = await fetch('https://dall-e-07kz.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type':'application/json',
          },
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          <FormField labelName="Prompt"
            type="text"
            name="prompt"
            placeholder="sponge bob surfing"
            value={form.prompt}
            handleChange={handleChange}
            isSurpriseMe
